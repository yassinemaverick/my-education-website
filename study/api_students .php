...
try {
    ...
    if ($isAdmin) {
        ...
        if ($action === 'update_email') {
            ...
            if ($check->fetch()) { echo json_encode(['ok'=>false,'error'=>'Cet email est déjà utilisé.']); exit; }
        ...
        if ($action === 'create_user') {
            ...
            if (!$fullname || !$username || mb_strlen($pw) < 8) {
                echo json_encode(['ok'=>false,'error'=>'Champs requis manquants ou mot de passe trop court.']); exit;
            }
            if (!in_array($role_new, ['student','teacher'])) {
                echo json_encode(['ok'=>false,'error'=>'Rôle invalide.']); exit;
            }
            ...
            if ($chk->fetch()) { echo json_encode(['ok'=>false,'error'=>'Cet identifiant est déjà utilisé.']); exit; }
            $hash = password_hash($pw, PASSWORD_BCRYPT, ['cost'=>12]);
            try { $pdo->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS email VARCHAR(180) DEFAULT NULL"); } catch(Throwable $e){}
        ...
        if ($action === 'registered_students') {
            ...
            if ($search) {
                $like = "%{$search}%";
                $stmt = $pdo->prepare("SELECT id, full_name, username, email, created_at FROM users WHERE role='student' AND (full_name LIKE ? OR username LIKE ? OR email LIKE ?) ORDER BY full_name");
                $stmt->execute([$like, $like, $like]);
            } else {
                $stmt = $pdo->prepare("SELECT id, full_name, username, email, created_at FROM users WHERE role='student' ORDER BY full_name");
                $stmt->execute();
            }
            $count = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role='student'")->fetchColumn();
            echo json_encode(['ok'=>true,'students'=>$stmt->fetchAll(),'total'=>$count], JSON_UNESCAPED_UNICODE);
            exit;
        }
        if ($action === 'enrollments') {
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS enrollments (
                ...
                status     ENUM('new','refused','accepted') NOT NULL DEFAULT 'new',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_status (status),
                INDEX idx_created (created_at)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
            // Migrate old status values if table existed with different enum
            try { $pdo->exec("ALTER TABLE enrollments MODIFY COLUMN status ENUM('new','refused','accepted') NOT NULL DEFAULT 'new'"); } catch(Throwable $e){}
            ...
            echo json_encode(['ok'=>true,'rows'=>$stmt->fetchAll(),'counts'=>$countMap], JSON_UNESCAPED_UNICODE);
            exit;
        }
    ...
    if (!$isTeacher) { http_response_code(403); echo json_encode(['ok'=>false,'error'=>'Unauthorized']); exit; }
